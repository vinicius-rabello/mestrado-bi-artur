...
for municipio in tqdm(municipios):
    ...
    WITH censo_municipio AS (
        ...
        SELECT
            COUNT(DISTINCT(fogo_id)) AS num_domicilios,
            SUM(n_escravizados) AS total_escravizados
            FROM com_fogo_id
            WHERE id = fogo_id AND n_escravizados > 0
    ), with_qual_n_esc AS (
        SELECT
            *,
        ...
        FROM com_fogo_id
        WHERE n_escravizados > 0 AND id = fogo_id
    ), com_porcentagens AS (
        SELECT
        ...
        SELECT
            ...
            MAX(ROUND(SUM("Porcentagem de Domicílios") OVER (
            ...
            MAX(ROUND(SUM("Porcentagem de Escravos") OVER (
...
with pd.ExcelWriter("novas_tabelas/tabela_porcentagens_domicilios_escravos_31.xlsx", engine="xlsxwriter") as writer:
