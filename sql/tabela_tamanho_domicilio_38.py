...
for municipio in tqdm(municipios):
    query = f"""
        ...
        FROM Censo38_escravizados
        WHERE Município = '{municipio[0]}' AND Relação = 1
    ), estatisticas AS (
        SELECT 
            possui_escravos,
...
with pd.ExcelWriter("novas_tabelas/tabela_tamanho_domicilio_38.xlsx", engine="xlsxwriter") as writer:
