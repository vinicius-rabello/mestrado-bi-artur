...
def contar_agregados(row):
    ...
    if 'Agregado:' in obs or 'Agregados:' in obs:
        # ignorar o que vem a partir de Testemunha
        agregados = obs.split('Testemunha:')[0]
        # extrair o que vem após os dois pontos
        agregados = agregados.split(':')[-1]
        # organizar os delimitadores
        agregados = agregados.replace(' e ', ', ')
