...
def gerar_possiveis_matches(path, municipio=None):
    ...
    if municipio:
        entradas_recibos = cursor.execute(
            f"SELECT DISTINCT Arrecadador, Município, Freguesia, Distrito, id, Data, IdArrecadador38, Localização FROM Recibos \
                WHERE Município = '{municipio}';").fetchall()
    else:
        entradas_recibos = cursor.execute(
            f"SELECT DISTINCT Arrecadador, Município, Freguesia, Distrito, id, Data, IdArrecadador38, Localização FROM Recibos;").fetchall()
        ...
        for entrada_censo in tqdm(entradas_censo38):
            ...
            if sim >= 0.8:
                possiveis_matches.append(
                    [arrecadador, nome, municipio_recibo, distrito_recibo, freguesia_recibo, localizacao_recibo,
...
gerar_possiveis_matches('database/tables/possiveis_matches_arrecadador_38.xlsx')
